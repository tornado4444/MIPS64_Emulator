...
// Bounds checking
static bool mips64_bound_checking(uint64_t address, uint64_t data_size) {
...
Mips64MemoryStatus mips64_memory_read16(const Memory* mem, uint64_t address, uint16_t* out_value) {
	...
	if(!mips64_alignment_checking(address, 2)) {
...
Mips64MemoryStatus mips64_memory_read32(const Memory* mem, uint64_t address, uint32_t* out_value){
	...
	if(!mips64_alignment_checking(address, 4)) {
		return MIPS64_MEMORY_MISALIGNED;
	}
...
Mips64MemoryStatus mips64_memory_read64(const Memory* mem, uint64_t address, uint64_t* out_value){
	...
	if(!mips64_alignment_checking(address, 8)) {
		return MIPS64_MEMORY_MISALIGNED;
	}
...
// For write
Mips64MemoryStatus mips64_memory_write8(Memory* mem, uint64_t address, uint8_t value) {
	if(mem == NULL) {
		return MIPS64_MEMORY_NULL_POINTER;
	}	
...
Mips64MemoryStatus mips64_memory_write16(Memory* mem, uint64_t address, uint16_t value){
	if(mem == NULL) {
		return MIPS64_MEMORY_NULL_POINTER;
	}	
...
Mips64MemoryStatus mips64_memory_write32(Memory* mem, uint64_t address, uint32_t value){
	if(mem == NULL) {
		return MIPS64_MEMORY_NULL_POINTER;
	}	
...
Mips64MemoryStatus mips64_memory_write64(Memory* mem, uint64_t address, uint64_t value) {
	if(mem == NULL) {
		return MIPS64_MEMORY_NULL_POINTER;
	}
	...
	if(mem->endian == MIPS64_ENDIAN_LITTLE) {
		for (uint32_t i = 0; i < 8; ++i) {
			mem->memory[address + i] = (uint8_t)((value >> (i * 8u)) & UINT64_C(0xFF));
		}
	} else {
		for (uint32_t i = 0; i < 8; ++i) {
